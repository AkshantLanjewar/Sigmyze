...
use super::{sdmx_xsd_data::SDMXSeriesFields, sdmx_series::{self, SDMXSeries}};
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Start(e)) => {
                ...
                match tag_name {
                    "Series" => {
                        let mut sdmx_series = sdmx_series::SDMXSeries::new();
                        for attribute in e.attributes().into_iter() {
                            let attribute = attribute.unwrap();

                            let attr_name = &attribute.key;
                            let attr_name = String::from_utf8(attr_name.as_ref().to_vec()).unwrap();

                            let attr_value = &attribute.value;
                            let attr_value = String::from_utf8(attr_value.as_ref().to_vec()).unwrap();

                            let xsd_series = xsd_defs.get_series_name(&attr_name);
                            if xsd_series.is_some() {
                                ...
                                for xsd_value in xsd_values.iter() {
                                    ...
                                    if attr_value.as_str() == field_value {
                                        ...
                                        sdmx_series.add_field(attr_name.clone(), field_value, field_doc);
                                    }
                                }
                            }
                        }
            ...
            Ok(Event::Empty(e)) => {
                ...
                match tag_name {
                    "Obs" => {
                        ...
                        let time_period = SDMXSeries::parse_date(&frequency, &time_period);
