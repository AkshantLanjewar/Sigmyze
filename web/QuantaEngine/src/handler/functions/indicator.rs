...
#[derive(Debug, Deserialize, Serialize)]
pub struct UpdateIndicatorBody {
...
pub async fn update_indicator(
    ...
    let body: UpdateIndicatorBody = serde_json::from_str(&function_data).expect("bad_body");
    let mode = match body.mode {
        Some(v) => v,
        None => return Err("no_update_mode".into())
    };
    ...
    let cache_str = format!("{}::{}", mode, indicator_str);
    let cache_key = format!("{}::cache", process_id);
    append_cache(cache_key, cache_str, store).await;
